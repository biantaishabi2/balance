...
**模块一：期末结账 + 凭证审核流程**
- 凭证流转：draft → reviewed → confirmed
- 结账：期间关闭、损益结转、本期结转到下期
- 关键命令：`ledger review`/`ledger unreview`/`ledger close`/`ledger reopen`

**模块二：往来核销 + 存货核算 + 固定资产**
- 往来核销：应收/应付明细、核销记录、账龄查询
- 存货核算：入库/出库、移动平均成本、结存
- 固定资产：资产卡片、折旧计提、处置
